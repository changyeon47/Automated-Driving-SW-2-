...
rclcpp::Publisher<std_msgs::msg::String>::SharedPtr mypub)//함수 정의
{
    auto message = std_msgs::msg::String();//객체 생성
    message.data = "Hello world!";//객체 맴버변수 값 초기화
    RCLCPP_INFO(node->get_logger(), "Publish: %s", message.data.c_str());//콘솔창에 출력
...
int main(int argc, char*argv[]) //메인함수
{
    ...
    auto mypub=node->create_publisher<std_msgs::msg::String>("topic_pub1", qos_profile);//pub객체 생성
